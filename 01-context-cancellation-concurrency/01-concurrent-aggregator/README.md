...
You need to fetch these in parallel to reduce latency. However, if _either_ fails, or if the global timeout is reached, the entire operation must abort immediately to save resources.
...
2.  **The "Domino Effect":**
    ...
    - **Pass Condition:** Does your function return the error _immediately_? (If it waits 10s, you failed context cancellation).
